...
def create_model(args, num_labels, model_class_fn, tokenizer):
  ...
  model = model_class_fn(init_model, args.model_config, num_labels=num_labels, \
      drop_out=args.cls_drop_out, args=args, \
...
def train(args, parser=None):
  if parser is None:
    ...
    for action in parser._actions:
      if action.required and action.dest not in args:
        raise ValueError(f'Argument {action.dest} is required.')
      if not action.required and action.dest != "help":
        if action.dest not in args:
          args[action.dest] = action.default

    args = make_dataclass('Args', args.keys())(**args)
