...
class DistributedTrainer:
  ...
  def train(self):
    ...
    self.trainer_state.pbar = tqdm.tqdm(total=self.training_steps, desc='Training', ncols=100, smoothing=0.05)
  ...
  def save_model(self, args, save_path, model):
  ...
  def _eval_model(self, with_checkpoint=True):
    if with_checkpoint:
      ...
      self.save_model(self.args, save_path, self.model)
    ...
    if self.eval_fn is not None:
      ...
      if metric is not None:
        if metric > _metric:
          ...
          self.save_model(self.args, os.path.join(self.output_dir, 'pytorch_model.bin'), self.model)
